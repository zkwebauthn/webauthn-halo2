...
impl<F: PrimeField> Default for ES256Circuit<F> {
    fn default() -> Self {
        Self {
            r: None,
            s: None,
...
impl<F: PrimeField> Circuit<F> for ES256Circuit<F> {
...
fn test_secp256r1_ecdsa() {
    ...
    let msg = <Secp256r1Affine as CurveAffine>::ScalarExt::random(OsRng);
    ...
    let s = k_inv * (msg + (r * sk));

    let circuit = ES256Circuit::<Fr> {
        r: Some(r),
        s: Some(s),
        msg: Some(msg),
        pk: Some(pubkey),
...
fn bench_secp256r1_ecdsa() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for line in bench_params_reader.lines() {
        ...
        let circuit = ES256Circuit::<Fr>::default();
        ...
        let proof_circuit = ES256Circuit::<Fr> {
            r: Some(r),
            s: Some(s),
            msg: Some(msg),
            pk: Some(pubkey),
