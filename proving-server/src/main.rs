...
fn generate_evm_verifier(request_body: Json<GenerateEVMVerifierRequestBody>) -> Vec<u8> {
    let (bytes, yul_code) = generate_verifier(&request_body.verifying_key_path, DEGREE).unwrap();
    println!("{}", yul_code);
    bytes

    // let mut f = File::create(&request_body.sol_code_path.as_ref().unwrap())?;
    // let _ = f.write(yul_code.as_bytes());

    // let output = fix_verifier_sol(&request_body.sol_code_path.as_ref().unwrap().clone())?;
